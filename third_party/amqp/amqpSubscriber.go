...
func newAmqpSubscription(queueName, key, exchange string, autoAck bool, workerCount int, processor messaging2.SubscribeProcessor) *amqpSubscriber {
	return &amqpSubscriber{
		...
		processor:   processor,
		Logger:      logger.Log.WithFields(logger.BuildContext("amqpSubscription")).WithField("queueName", queueName),
	}
}
...
func (sub *amqpSubscriptionManager) CreateSubscription(queueName, key, exchange string, autoAck bool, workerCount int, processor messaging2.SubscribeProcessor) (messaging2.Subscription, error) {
...
type amqpSubscriber struct {
	...
	processor    messaging2.SubscribeProcessor
	stopChan     chan int
...
func (c *amqpSubscriber) runEventProcessor(deliveryChan <-chan amqp.Delivery) {
	pool := third_party.NewIntPool(c.workerCount)
	...
	for {
		...
		case d := <-deliveryChan:
			...
			go func(worker int, event *amqpEvent, log logger.Logger) {
				logger := log.WithField("workerNumber", worker).
					WithField("correlationId", event.GetCorrelationID())

				ctx := third_party.NewSessionCtx(event.GetCorrelationID(), logger)
