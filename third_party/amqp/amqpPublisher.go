...
func (pub *amqpPublisherManager) CreatePublisher(publishKey string) (messaging2.Publisher, error) {
	logger.Log.WithField("publishKey", publishKey).Info("create publisher")
...
func (client *amqpPublisher) init(connection *amqp.Connection) error {
	logger.Log.Infof("Amqp publisher initiated %s", client.publishKey)
